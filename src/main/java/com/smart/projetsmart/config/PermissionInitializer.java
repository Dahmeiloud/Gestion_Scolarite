...
public class PermissionInitializer implements ApplicationRunner {
    ...
    public void createSuperUser(String username, String password, String email) {
        Optional<UserApp> user1 = userInfoRepository.findByUsername(username);
        Optional<UserApp> user2 = userInfoRepository.findByEmail(email);
        if (user1.isPresent() || user2.isPresent()) {
            UserApp user = user1.isPresent() ? user1.get() : user2.get();
